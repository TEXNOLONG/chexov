...
export const THEMES: ThemeDef[] = [
  { id: 'dark-purple', label: 'Ночной',   preview: ['#07050e', '#a78bfa', '#110d22'] },
  { id: 'midnight',    label: 'Полночь',  preview: ['#06060a', '#60a5fa', '#0f1120'] },
  { id: 'sunset',      label: 'Закат',    preview: ['#100800', '#f59e0b', '#1c1106'] },
  { id: 'forest',      label: 'Лес',      preview: ['#06100a', '#34d399', '#0c1f15'] },
  { id: 'light',       label: 'Светлая',  preview: ['#f5f5f7', '#7c3aed', '#ffffff'] },
]
...
export function loadThemeId(): ThemeId {
  ...
  return (v && THEME_VARS[v as ThemeId]) ? v as ThemeId : 'dark-purple'
}

export function applyTheme(id: ThemeId) {
  let el = document.getElementById('chexov-theme') as HTMLStyleElement | null
  if (!el) { el = document.createElement('style'); el.id = 'chexov-theme'; document.head.appendChild(el) }
  el.textContent = `:root { ${THEME_VARS[id]} }`

  const bgMap: Record<ThemeId, string> = {
    'dark-purple': `
      radial-gradient(ellipse 110% 65% at 5% 0%,rgba(110,55,255,0.28) 0%,transparent 60%),
      radial-gradient(ellipse 80% 55% at 95% 100%,rgba(80,30,200,0.18) 0%,transparent 55%),
      radial-gradient(ellipse 60% 80% at 50% 45%,rgba(55,20,160,0.14) 0%,transparent 70%)`,
    'midnight': `
  ...
  document.body.style.backgroundImage = bgMap[id]
  document.body.style.backgroundColor = id === 'light' ? '#f5f5f7' : ''
}

interface ThemeCtx { themeId: ThemeId; setTheme: (id: ThemeId) => void }
const ThemeContext = createContext<ThemeCtx | null>(null)

export function ThemeProvider({ children }: { children: ReactNode }) {
  const [themeId, setThemeIdState] = useState<ThemeId>(() => {
    const id = loadThemeId()
    applyTheme(id) // apply synchronously before first paint
    return id
  })
