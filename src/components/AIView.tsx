...
${menu
  .map((item) => `${item.name} (${item.category}) — ${item.price > 0 ? formatPrice(item.price) : 'цена не указана'}${item.description ? ': ' + item.description.slice(0, 120) : ''}${item.allergens ? ` [Аллергены: ${item.allergens}]` : ''}`)
  .join('\n')}
...
export function AIView() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev.slice(0, -1),
        { role: 'assistant', content: '⚠️ Ошибка соединения. Проверьте интернет и попробуйте снова.' },
      ])
    } finally {
      setLoading(false)
    }
  ...
  async function generateImage() {
    ...
    try {
      const encoded = encodeURIComponent(`${prompt}, restaurant food photography, professional, warm lighting`)
      ...
      setMessages((prev) => [
        ...prev.slice(0, -1),
        { role: 'assistant', content: '⚠️ Не удалось сгенерировать изображение. Попробуйте снова.' },
      ])
    } finally {
      setLoading(false)
    }
    ...
    <div className="flex h-full flex-col">
      ...
      {showImageGen && (
        <div className="mb-3 flex gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 animate-slide-up">
          ...
          <button type="button" onClick={() => setShowImageGen(false)} className="text-[var(--muted)]">✕</button>
        </div>
      )}
      ...
      <div className="flex items-end gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2">
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 resize-none bg-transparent py-1 text-sm outline-none placeholder:text-[var(--muted)]"
      ...
      <p className="mt-2 text-center text-[10px] text-[var(--muted)]">AI работает бесплатно через Pollinations.ai</p>
    </div>
  )
}
